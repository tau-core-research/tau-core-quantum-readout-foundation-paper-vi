...
\date{Last revised: 29 August 2026}
...
We further derive a local observer-resolution window: a nonzero continuous
precursor becomes a distinct record only when its complete record jet exits a
source-owned stable quantizer cell before the local geometric approximation
fails.  On a constant-rank access stratum, an exhaustive complete
effect--Choi instrument detects every nonzero outward Kato projector jet,
although instantaneous probabilities and incomplete record families may
remain blind.  Finally, simultaneous Parent record maps give nested
observer-relative history fibres.  A common pre-choice preparation and a
normalized later instrument preserve the earlier marginal, so delayed-choice
conditioning refines histories without rewriting a Parent past or enabling
retro-signalling.
...
the Standard Model.  In particular, the quantum projective measure is not
identified with body four-volume by rank equality alone; a full source-owned
projective/body connector remains an independent premise.  A braid-holonomy
audit similarly gives an exact conditional anyon-readout criterion but proves
that the current source does not select its topology or representation.  The
same audit isolates a source-frozen three-mode susceptibility target; it is a
conditional precursor, not a reported experimental prediction. Finally, the
paper supplies the quantum operational obligations for observer-dependent
cross-terminal mixing: information-level role exchange does not identify a
contextual quantum packet with a classical noncontextual gravity terminal.
\end{abstract}
...
status ledger. It also owns the local quantizer-resolution window, the
outward-access/exhaustive-Choi detection theorem, the atemporal
history-refinement boundary, the recovery-based factor selector, its
...
Paper V's temporal-ontology underdetermination is inherited unchanged: when
the quantum and record terminals factor through the observer-accessible
restriction of one completed body, they cannot distinguish that body from a
coherent staged realization with the same restrictions.  Quantum ambiguity,
unitary transport or outcome records therefore may not be used backwards to
infer literal parent morphogenesis or an external parent time.
\end{minipage}}
\end{center}

\paragraph{Quantum specialization of the unified terminal atlas.}
For the shared preterminal descriptor \(\Xi_{OS}\) owned upstream, the quantum
terminal has the inherited form
\begin{equation}
 Y_{OS}^{(Q)}
 =
 Q_{OS}^{(Q)}\circ T_{OS}^{(Q)}
 \circ\Xi_{OS}\circ A_{OS}[M_\tau].
 \label{eq:quantum-common-ancestor-specialization}
\end{equation}
Its distinctive content is not a second source but the phase/effect,
composition, tensor and stable-resolution structures retained by
\(T_{OS}^{(Q)}\) and \(Q_{OS}^{(Q)}\).  A hybrid terminal is likewise a
multi-leg descendant,
\begin{equation}
 Y_{OS}^{(H)}
 =
 Q_{OS}^{(H)}
 \circ H_{OS}\!\left(
   T_{OS}^{(a_1)}\Xi_{OS},\ldots,T_{OS}^{(a_k)}\Xi_{OS}
 \right),
 \label{eq:hybrid-common-ancestor-specialization}
\end{equation}
not an independent parent channel.  Common ancestry does not identify
contextual quantum structure with a classical noncontextual terminal:
operational equivalence still requires preservation of composition, effects,
phases, probabilities and no-signalling structure.  A terminal varying along
a \(\Xi_{OS}\)-fibre would instead be a common-ancestor bypass and would
demote the declared atlas-completeness claim.

The quantum terminal also inherits, rather than creates, the visible
four-dimensional carrier.  A possible upstream representation uses a common
body-side $*$-derivation family $\mathfrak d_\tau$, a source-owned access
restriction $R^{\mathfrak d}_{OS}$ and
\[
 \Phi_{OS,x}=\widetilde\Phi_{OS,x}\circ R^{\mathfrak d}_{OS}.
\]
Neither finite resolution, non-injectivity, phase retention nor the Born
packet forces $\operatorname{rank}\Phi_{OS,x}=4$, its integrability, or a
Lorentz-conformal realization.  Conversely, a regular rank-four quotient
does not supply coherent composition, effects, tensor structure or
no-signalling.  Quantum recovery and 4D reconstruction are therefore
distinct conditional gates joined only by a source-owned common carrier.
...
\textbf{Exterior source-grammar refinement.}
Let $V_s$ be an oriented primitive possibility space. If primitive directions
are multiplicity-free, so that $v^2=0$, bilinear polarization gives
$uv=-vu$. With a unit and no additional source relation, the complete finite
grammar is therefore $\Lambda^\bullet V_s$, and the empty word is its
distinguished root. A positive base metric then supplies insertion and
contraction operators and hence the finite Clifford/cyclic carrier used below.
Tensor and symmetric grammars are explicit controls: primitive inventory and
composition alone do not select the exterior branch. The result is a
conditional source-class theorem, not proof that Nature occupies that class.
\end{minipage}}
\end{center}
...
Separator handoff & One coherent $\mathfrak J_*=(\Pi_*,\varphi_*)$ fixes the
mixed ideal, multiplicity pairing, relative $P3$--$P4$ gluing and faithful
block occupation & Conditional joint-source compression; current narrow-source
selection and global terminal completeness open \\
\bottomrule
\end{tabularx}
...
The label $O$ initially denotes a body-side carrier candidate.  Operational
observerhood and an accessible four-dimensional world arise together only
when the same solution has regular local rank four, a stable quantizer and a
nonzero record/effect.  Hence information loss is not transmission toward a
detached observer, and a 4D chart alone is not a measurement context.

Paper VI inherits, rather than proves, the minimal formation factorization.
The base is the protected zero-source response structure of one joint law, the
universal seed is a response-distinct base-relative source class, and
$M_\tau$ is a stable selected nonneutral response.  The split is separately
identifiable only after a physical zero-source member is fixed.  Quantum
effects, Born statistics and process tomography are downstream and cannot
define that anchor or construct the seed backward from terminal data.
...
This co-descent interpretation sharpens the quantum claim boundary.  Finite
observer-relative quantization belongs to the formation of operational
readout classes, but it does not by itself imply complex phase composition,
Born probabilities, tensor products, entanglement or no-signalling.  Those
remain additional source-owned structures derived conditionally below.
...
\section{Local transition resolvability and record-jet detection}

The stable quantizer makes the smallest operational transition a derived
observer--source scale, not an infinitesimal property of the continuous
carrier.  Let $y(\sigma)$ be a source-frozen differentiable local ordered
precursor and put
\begin{equation}
 v=y'(\sigma_0)\ne0,
 \qquad
 d_e(\delta)=y(\sigma_0+\delta)-y(\sigma_0).
 \label{eq:local-ordered-chord}
\end{equation}
Continuity gives, for all sufficiently small $\delta>0$,
\begin{equation}
 \langle d_e(\delta),v\rangle
 >{\delta\over2}\lVert v\rVert^2>0,
 \qquad
 {d_e(\delta)\over\delta}\longrightarrow v.
 \label{eq:local-positive-overlap}
\end{equation}
Thus a short positive chord has positive tangent overlap and asymptotically
unit alignment.  This statement is local: a long curved chord may be
orthogonal to its initial tangent.

Let $\mathcal R_{OS}$ be the complete source-frozen record descriptor and let
$C_0$ be the stable $Q_{OS}$ cell containing
$r_0=\mathcal R_{OS}(0)$.  Assume, in one inherited norm,
\begin{equation}
 B_{r_-}(r_0)\subseteq C_0\subseteq B_{r_+}(r_0),
 \qquad 0<r_-\le r_+,
 \label{eq:stable-cell-radii}
\end{equation}
and along Eq.~\eqref{eq:local-ordered-chord}
\begin{equation}
 \mathcal R_{OS}(d_e(\delta))-r_0
 =\delta a+\eta(\delta),
 \qquad
 a=D\mathcal R_{OS}|_0(v),
 \qquad
 \lVert\eta(\delta)\rVert\le {C\over2}\delta^2.
 \label{eq:record-jet-expansion}
\end{equation}

\begin{proposition}[Stable-cell resolution window]
\label{prop:stable-cell-window}
Under Eqs.~\eqref{eq:stable-cell-radii}--\eqref{eq:record-jet-expansion},
\begin{align}
 \delta\lVert a\rVert+{C\over2}\delta^2<r_-
 &\Longrightarrow Q_{OS}(d_e(\delta))=Q_{OS}(0),
 \label{eq:definitely-unresolved}\\
 \delta\lVert a\rVert-{C\over2}\delta^2>r_+
 &\Longrightarrow Q_{OS}(d_e(\delta))\ne Q_{OS}(0).
 \label{eq:definitely-resolved}
\end{align}
For $C>0$ and $\lVert a\rVert^2>2Cr_+$, the smaller sufficient exit scale is
\begin{equation}
 \delta_Q^+
 ={\lVert a\rVert-\sqrt{\lVert a\rVert^2-2Cr_+}\over C}.
 \label{eq:quantizer-exit-scale}
\end{equation}
If the local geometric result holds to $\delta_{\rm loc}$, a nonempty
resolved-local window exists when $\delta_Q^+<\delta_{\rm loc}$ and
$\delta_Q^+<\delta<\delta_{\rm loc}$ is chosen on the increasing branch.
\end{proposition}

\begin{proof}
The triangle inequalities applied to
Eq.~\eqref{eq:record-jet-expansion} give
\[
 \delta\lVert a\rVert-{C\over2}\delta^2
 \le\lVert\mathcal R_{OS}(d_e(\delta))-r_0\rVert
 \le\delta\lVert a\rVert+{C\over2}\delta^2.
\]
The inner inclusion in Eq.~\eqref{eq:stable-cell-radii} proves
Eq.~\eqref{eq:definitely-unresolved}; the outer inclusion proves
Eq.~\eqref{eq:definitely-resolved}.  Solving equality in the latter lower
bound gives Eq.~\eqref{eq:quantizer-exit-scale}.
\end{proof}

Consequently nonzero continuous change is not yet time or quantum
quantization.  The physical chain must source both the record jet and the
stable partition,
\begin{equation}
 (K_{\rm src},J_{OS}^{\rm acc})\longrightarrow P_O
 \longrightarrow\{\mathcal I_a\}_a
 \longrightarrow\mathcal R_{OS}\longrightarrow Q_{OS}^{\mathcal I}.
 \label{eq:instrument-quantizer-chain}
\end{equation}
At constant rank define
\begin{equation}
 B(M)=J_{OS}^{\rm acc}(M)K_{\rm src}^{-1/2},
 \quad
 P_O=B^\dagger(BB^\dagger)^+B,
 \quad W=B^\dagger.
 \label{eq:access-projector}
\end{equation}
Its Kato projector jet in direction $v$ is
\begin{equation}
 D_vP_O=X_v+X_v^\dagger,
 \qquad
 X_v=(I-P_O)(D_vW)W^+.
 \label{eq:kato-outward-jet}
\end{equation}
An internal basis change of the access image can have $D_vW\ne0$ but
$X_v=0$; base incidence and its metric therefore do not determine outward
motion.  If $W=SR$, the correctly typed nonstandard candidate is the
standard-access-quotiented covariant second fundamental form
\begin{equation}
 \mathrm{II}_{OS}^{\perp}(v)
 =(I-\Pi_{\rm std}^{\rm acc})(I-P_O)S\nabla_v^{(K)}R.
 \label{eq:observer-second-fundamental-form}
\end{equation}
Its nonvanishing guarantees nonstandard outward access motion, not by itself
finite-cell exit.

\begin{theorem}[Exhaustive complete-Choi detection]
\label{thm:exhaustive-choi-detection}
Let $X_v\ne0$, $G_v=X_v-X_v^\dagger$, and let an owned finite instrument have
effect roots $S_a=\sqrt{F_a}$ exhaustive on the accessible support,
\begin{equation}
 \sum_aF_a=\sum_aS_a^2=P_O.
 \label{eq:exhaustive-effects}
\end{equation}
Under Kato transport, $\dot K_a=G_vS_a$ and
$J_a=|K_a\rangle\!\rangle\langle\!\langle K_a|$.  Then
\begin{equation}
 X_v\ne0\quad\Longrightarrow\quad
 \dot J_a\ne0\ \text{for at least one }a.
 \label{eq:choi-detection}
\end{equation}
The immediate effects may nevertheless obey $\dot F_a=0$.
\end{theorem}

\begin{proof}
The ranges of $S_a$ and $G_vS_a=X_vS_a$ are orthogonal, so
$\dot J_a=0$ exactly when $X_vS_a=0$.  If all Choi derivatives vanished,
exhaustivity would give
\[
 X_v=X_vP_O=X_v\sum_aS_a^2
 =\sum_a(X_vS_a)S_a=0,
\]
a contradiction.  Orthogonality also gives
$\dot F_a=(G_vS_a)^\dagger S_a+S_aG_vS_a=0$.
\end{proof}

\begin{corollary}[Full-support and scalar-phase controls]
\label{cor:full-support-scalar-controls}
If the direct access realization has \(P_O=I_n\), then
\begin{equation}
 X_v=(I-P_O)(D_vW)W^+=0,
 \qquad G_v=X_v-X_v^\dagger=0,
 \label{eq:full-support-kato-zero}
\end{equation}
so the Kato branch contains no outward access transport even when the terminal
process varies.  If instead a Kraus amplitude is transported only by a scalar
generator \(G_v=ia(v)I_n\), its normalized Choi projector is unchanged by the
resulting global phase and
\begin{equation}
 q_{\rm pu}(G_v)
 :=G_v-\frac{\operatorname{tr}G_v}{n}I_n=0.
 \label{eq:scalar-choi-null}
\end{equation}
Therefore a non-scalar normalized-Choi orientation requires either the
internal component of a fuller body connection or a genuinely varying access
projector.
\end{corollary}

\begin{proof}
The first statement follows directly from Eq.~\eqref{eq:kato-outward-jet}.
For the second, scalar transport multiplies every Kraus vector by one phase;
the outer product \(\lvert K\rangle\!\rangle\langle\!\langle K\rvert\) is
phase invariant, and the displayed projective quotient vanishes identically.
\end{proof}

Probability-only observers and incomplete Choi families can therefore miss
an outward access jet.  Conversely, exhaustive detection does not make the
signal Tau-specific: the additional discriminator is a nonzero component
after projection away from independently frozen standard apparatus Choi
tangents.  Even that infinitesimal component must still satisfy
Proposition~\ref{prop:stable-cell-window} to become a finite resolved record.
Physical joint occupation of the front, incidence, complete instrument and
stable cell remains open.
...
\section{Braid holonomy and the body--observer metric handoff}

Paper III's path-holonomy theorem has an exact quantum specialization.  Let
$\Sigma_{OS}$ be an independently established oriented two-dimensional
effective support, let coincidences be excluded, and let a flat occupied
Hermitian channel connection descend to $\rho_{OS}:B_n\to U(k)$.  The readout
is anyonic if and only if this representation is nontrivial after the
source-derived quantizer and terminal null quotient.  Abelian statistics has
$k=1$; genuinely non-Abelian statistics requires noncommuting adjacent braid
generators.  The current Tau source does not force the two-dimensional
support, coincidence gap, flat connection, nontrivial representation or
terminal visibility.  This conditional theorem must therefore not be read
backward as source selection.

The body quotient nevertheless supplies a quantitative, source-side
compression question.  On the rooted relative carrier write
\begin{equation}
 K_{\rm leaf}=k_RP_R+k_TP_T,
 \qquad k_R=0.777537432703,
 \quad k_T=1.072168783649,
 \label{eq:leaf-stiffness-values}
\end{equation}
where $P_R$ is the rooted singlet and $P_T$ the tangential doublet.  These are
three-dimensional body-stabilization eigenvalues; they do not establish a
physical two-dimensional surface.  For observer access
$R_O=a_RP_R+a_TP_T$, exact quadratic compression gives
\begin{equation}
 K_{{\rm occ},O}=(R_OK_{\rm leaf}^{-1}R_O^*)^{-1}.
 \label{eq:observer-compression}
\end{equation}
The tangential doublet is the softer resolved sector precisely when
\begin{equation}
 \frac{a_T}{a_R}>\sqrt{\frac{k_T}{k_R}}=1.174278004418.
 \label{eq:access-threshold}
\end{equation}
Symmetry, rank and positivity do not force this inequality; equal-reduct
positive counterpairs occur on both sides of the threshold.

The observer first jet can be factored as $J_O=-H_SL_S$, with
$H_S=h_RP_R+h_TP_T$ and $L_S=\ell_RP_R+\ell_TP_T$.  A coefficient-free
conditional completion is obtained only after the additional metric-copy
postulate
\begin{equation}
 H_S=K_{\rm leaf},\qquad L_S=I.
 \label{eq:metric-copy-completion}
\end{equation}
It yields $K_{{\rm occ},O}=K_{\rm leaf}^{-1}$ and the normalized
tangential-to-radial stiffness target
\begin{equation}
 \chi_{2D}=\frac{k_R}{k_T}=0.725200588341,
 \qquad g_N=1-\chi_{2D}=0.274799411659.
 \label{eq:smcf-target}
\end{equation}
The present body action and observer functional are a typed direct sum, so
their mixed Hessian vanishes.  Hence Eq.~\eqref{eq:metric-copy-completion} is
not a theorem of the current packet.  A minimal stationary graph source would
instead have quadratic germ
\begin{equation}
 \Phi_J^{(2)}=\frac12\langle v_A-U_{BA}c_B,
 H(v_A-U_{BA}c_B)\rangle,
 \qquad H=U_{BA}K_{\rm leaf}U_{BA}^*,
 \label{eq:joint-metric-copy-germ}
\end{equation}
where the existing carrier maps determine $U_{BA}$ but not the metric-copy
law.  We call its seed-level naturality requirement the seed metric-copy
functor (SMCF).  SMCF may act on every admissible observer--source module
without storing concrete future observer values in the universal seed, but
one-source identity and the currently proved closure conditions do not entail
it.  A positive metric-modulus counterfamily proves that independence.

An eligible endpoint would prepare three independently normalized rooted
perturbations, measure a fully calibrated static susceptibility matrix
$\chi$, whiten its nuisance metric, and form
$K_{\rm exp}=[(\chi+\chi^*)/2]^{-1}$.  SMCF predicts a tangential doublet and
the ratio in Eq.~\eqref{eq:smcf-target}.  A frequency ratio
$\omega_T/\omega_R=0.851587099680$ follows only if common scalar inertia is
independently established.  Public braiding, fusion, interference and
engineered-spectrum packages audited to date do not provide the required
source-owned calibrated $3\times3$ response, so none currently tests SMCF.
Arbitrary peak matching is inadmissible.

\subsection{Cross-observer access is not free terminal relabelling}

The joint-terminal analysis of Paper VII-A permits the same parent direction
to be quantum-visible for one observer and gravity-visible for another when
their source-owned access maps have the required fiber factorization. This is
an information-level statement. It does not make ``quantum'' an arbitrary
observer label. The terminal constructed here is typed by its effect and
instrument algebra, coherent composition, Born probabilities and
no-signalling law.

If an injective multiplicative cross-observer map identifies this terminal
with another terminal, it must preserve commutators. Hence a genuinely
noncommutative quantum fragment cannot be fully identified with a commutative
classical-gravity fragment by access change alone. At the more general process
level, a full faithful probability-preserving equivalence would transport any
classical noncontextual simplex representation back to the quantum fragment.
Certified contextuality or incompatibility therefore requires the matched
terminal to retain nonclassical operational structure under the same frozen
causal, dimension and memory controls.

Sequence dependence alone is insufficient because classical disturbance,
backreaction and memory can imitate it. The physical test must reconstruct
the common parent fibers, certify operational equivalences independently,
reject a bounded classical comparator and transport composition with one
source-frozen map. Paper VII-A owns the general multi-terminal theorem; the
present paper supplies the quantum signature that any proposed role exchange
must preserve.
...
\subsection{Atemporal history refinement and no retro-signalling}

Delayed-choice conditioning can be stated without a Parent history store.
Let $(\mathcal P,\mu)$ be one source-frozen atemporal Parent dilation and
pre-choice weight for the prepared ensemble.  For settings $a,b$, let
\begin{equation}
 X_a:\mathcal P\to\mathsf X,
 \qquad D_b:\mathcal P\to\mathsf D
 \label{eq:simultaneous-record-maps}
\end{equation}
be stable early and later record maps.  These are simultaneous typed maps,
not a claim that every incompatible observable has a global noncontextual
value.  The observer-compatible fibres obey
\begin{equation}
 \mathcal H_x^{(a)}=X_a^{-1}(x),
 \qquad
 \mathcal H_{x,d}^{(a,b)}
 =X_a^{-1}(x)\cap D_b^{-1}(d)
 \subseteq\mathcal H_x^{(a)}.
 \label{eq:history-refinement-fibres}
\end{equation}
This is refinement of a conditional description, not rewriting an earlier
Parent state.  Stable record invariance separately requires an unconditioned
later operation $\Lambda_b$ to satisfy
\begin{equation}
 \Lambda_b^*[f(X_a)]=f(X_a)
 \quad\text{for every }f\text{ in the early record algebra}.
 \label{eq:stable-record-invariance}
\end{equation}
Record invariance alone does not imply no-signalling: a later-setting-
dependent preparation weight $\mu_b$ can change early frequencies while
leaving the record map fixed.

With one common pre-choice $\mu$ and a normalized later-local kernel
$K_b(d|p)$, the joint law is
\begin{equation}
 P(x,d|a,b)=\int_{\mathcal P}
 {\bf1}_{X_a(p)=x}K_b(d|p)\,d\mu(p),
 \qquad \sum_dK_b(d|p)=1.
 \label{eq:parent-joint-record-law}
\end{equation}
Therefore
\begin{equation}
 \sum_dP(x,d|a,b)
 =\int {\bf1}_{X_a(p)=x}\,d\mu(p)
 =P(x|a),
 \label{eq:no-retro-marginal}
\end{equation}
independently of $b$.  In the quantum terminal this is the standard identity
\begin{equation}
 \sum_d\Tr[(E_x^a\otimes F_d^b)\rho]
 =\Tr[(E_x^a\otimes I)\rho].
 \label{eq:quantum-no-retro-marginal}
\end{equation}
Conditioning on $d$ may still expose complementary interference patterns.
For example, terms proportional to $+V\cos\phi_x$ and $-V\cos\phi_x$ can
occur in two postselected subensembles while cancelling in their earlier
marginal.  Thus later records refine the compatible fibre but do not change
the locally accessible earlier statistics or open a future-to-past
information channel.  Standard quantum mechanics already explains this
structure; its compatibility is not evidence for Tau Core.
...
\subsection{Projective quantum measure versus body volume}

The finite common-Gram identity above compares an action separation with an
Uhlmann chord on one occupied record pair.  It must not be read as a global
identification of quantum-state volume with body volume.  In the rank-four
source/body prototype, CCOC supplies only
\begin{equation}
 A_M:V_{OS}^{\rm rel}\longrightarrow S_B,
 \qquad \operatorname{rank}A_M=3,
 \label{eq:quantum-imported-ccoc-map}
\end{equation}
while OGAS supplies a compatible signed common line.  Their combined rank is
four, but no full source-owned map
\begin{equation}
 \Xi:T(\mathbb{CP}^{1}_{s}\times\mathbb{CP}^{1}_{b})|_U
 \longrightarrow E_T\oplus E_S
 \label{eq:quantum-projective-body-map}
\end{equation}
follows.  In particular, rank equality does not imply
$\Xi^*d\mu_{h_*}=d\mu_{FS}$.

If such a connector is independently supplied, preserves the declared
splittings and has $|\det A_M|=g_{FFL}^{3}$ in the fixed oriented volume
frames, then exact measure matching uniquely fixes its common-line Jacobian:
\begin{equation}
 \Xi_{\gamma_T}=\gamma_T\oplus A_M,
 \qquad \gamma_T=g_{FFL}^{-3}.
 \label{eq:quantum-conditional-measure-normalization}
\end{equation}
This conditional uniqueness removes a possible blockwise gain.  It neither
constructs $\Xi$ nor identifies the body volume operationally, and it does not
replace the independent finite-action/Uhlmann test
Eq.~\eqref{eq:tau-discriminator}.  A common-line rescaling leaves the CCOC
spatial/contact data invariant while changing four-volume, so physical
connector ownership and projective/body measure identification remain
upstream open questions. The absolute M4 unit frame is a different issue:
Paper V proves that the common clock--gravity--action packet leaves only
\begin{equation}
 (L_H,T_H,\mathcal A_*)
 \longmapsto
 (\sigma L_H,\sigma T_H,\sigma^2\mathcal A_*),
 \qquad \sigma>0,
 \label{eq:quantum-inherited-m4-metrology}
\end{equation}
with all dimensionless clock--ruler terminals invariant. One measured anchor
chooses an SI representative; this does not construct $\Xi$, fix the
finite-action/Uhlmann bridge, or determine a dimensionless quantum coupling.
...
\subsection{Execution-to-process ledger audit}

A second bounded census, inherited from Paper V's temporal laboratory route,
asks a distinct question: does a public experimental package join an actually
executed, signed and winding-resolved control waveform to physical boundary
keys, same-run tomographic shots, a complete QPT/Choi reconstruction and a
calibration/nuisance ledger? No package verified on 24 August 2026 passed all
of these gates. The public quantum-memory repository is the strongest
laboratory near-pass \citep{HsiangQMemRepo}; the NGQG and NMN releases are
process-rich but lack the required execution join
\citep{ZhangNGQGData,GiarmatziNMNRepo}. QDataSet supplies waveform-rich
synthetic evolution, while the IBM pulse archive supplies hardware pulse
programs and state-tomography workflows rather than the complete same-run
process record \citep{PerrierQDataSetRepo,IBMOpenSciencePulse2021}.

This does not weaken the complete-Choi detection theorem. It prevents a
retrospective common-source test: without an immutable join key, permutations
of waveform-to-process pairings preserve both released marginals. Combining
separate public repositories therefore cannot manufacture the missing
incidence. The minimum operational acquisition is one run-keyed
execution--boundary--QPT ledger; independent physical ownership of the Tau
source port remains an additional upstream condition.

\subsection{Complete-process/apparatus near-pass and the Nature boundary}

A complementary public-data question is now decidable: can one acquire a
physical informationally complete micro-process family together with a real
apparatus descriptor? Liu et al.\ answer this weaker question positively
\citep{LiuRobustLindbladian2026,LiuLindbladianData2026}. Their checksum-frozen
archive contains four IBM device/date/qubit-pair contexts. Each context uses a
fixed $16\times16$ two-qubit preparation/effect grid; the accompanying backend
snapshot retains chip coordinates, the coupling map, calibration properties
and partially public Hamiltonian parameters. Edge, non-edge, idle,
remote-gate and parameterized-gate records supply useful standard controls.

This packet does not establish occupation of the Kato branch derived above.
Let $v$ denote a source-frozen morphology/access tangent, let $j_v$ be its
predicted Choi derivative and let $N$ span the declared standard nuisance
tangents. A model-class-relative Nature certificate requires, among its
conjunctive conditions, a smooth independently measured constant-rank
access-projector family on common run keys, nonzero Kato activation, nuisance
novelty $(I-NN^+)j_v\ne0$, held-out mean/covariance agreement, source-mode and
internal-support-motion nulls, a source-derived stable quantizer-cell crossing
and independent replication. The public backend snapshot is a static standard
hardware description, not such a family. The release also omits shot arrays,
repeat-level source--process covariance, source-derived $Q_{OS}$ cells and a
Tau derivative frozen before the terminal process data.

The frozen seven-clause audit therefore yields

\begin{center}
\begin{tabular}{lc}
\toprule
fully passed clauses & $0/7$ \\
partial physical/method controls & $3/7$ \\
failed or non-evaluable clauses & $4/7$ \\
complete terminal-process occupation & observed \\
Tau Kato micro-kernel occupation & open \\
\bottomrule
\end{tabular}
\end{center}

The positive acquisition result is consequently method-level: rich physical
process data and an independently supplied standard apparatus descriptor can
coexist. It is not a Tau detection. In particular, the observed process
variation cannot be differentiated backward to define $v$ or $j_v$, and a
static coupling graph cannot be silently promoted to the outward projector
jet of Eq.~\eqref{eq:kato-outward-jet}. This strengthens the empirical handoff
while leaving the complete-Choi theorem conditional and Nature occupation
open.
...
The later multimode-fibre method control makes the access distinction exact.
In the direct transfer-as-access realization all measured square maps are full
rank, hence \(P_O=I_{110}\) and Corollary~\ref{cor:full-support-scalar-controls}
gives zero Kato transport although the normalized Choi process changes.  The
current scalar source-connection candidate is likewise projectively null for
that non-scalar change.  A full independently sourced body connection or a
varying, gapped source-owned access subspace would be sufficient in principle;
neither is physically realized by the current source packet.  Fitting a local
generator from the same terminal transfer path would test standard optical
factorization, not derive a Tau source arrow.
...
Here the \emph{future--ROOT bridge} is the paper's map between an oriented
future-compatible body fibre and the logical ROOT quantum carrier.  It is not
the upstream DA--ROOT/M4TR route-agreement map
\[
 U_{\rm DA\leftrightarrow ROOT}:
 T^{\rm vis}_{OS,x}\longrightarrow E^{\rm ROOT}_4,
\]
which would identify a derivation-reconstructed regular 4D tangent with the
independently constructed ROOT/M4TR Lorentz carrier.  Closing either bridge
does not close the other.  The latter must additionally preserve the physical
null quotient, order/time-role line and Lorentz-conformal structure, and its
source ownership remains open.
...
& Conditional theorem \\
Local resolved transition & A nonzero record jet leaves a source-owned stable
cell only inside the explicit window of
Proposition~\ref{prop:stable-cell-window}; infinitesimal precursor survival is
insufficient & Conditional theorem and quantizer nonselection boundary \\
Outward access detectability & Every nonzero outward Kato jet changes at least
one exhaustive complete Choi record, while probabilities and incomplete
records may remain blind; standard-apparatus quotient and physical occupation
remain open & Conditional theorem plus exact controls \\
History refinement/no retro-signalling & Later records define nested Parent
preimage fibres; a common pre-choice weight and normalized later-local
instrument preserve the early marginal, whereas record invariance alone does
not & Conditional theorem and countermodel boundary \\
...
future fibre; physical orbit occupation and relative mode normalization remain open
...
The operational source-generated-completeness handoff fixes the domain of
this statement. Under the enriched one-source premise, the quantum packet is
defined on the seed-generated occupied physical quotient; inert ambient
summands are true-null presentation surplus. For the complete joint
relation/state packet this requires its selector to be constant on complete
seed fibres, so that the ideal and faithful state factor through the seed
key. The present narrow seed does not prove that factorization. A non-null
quantum difference at fixed complete seed key is instead a second source or
an incomplete seed descriptor and falsifies the one-source completion.

\subsection*{Pointed-exterior support inherited from the parent packet}

The later parent-packet analysis supplies a sharper support result.  On a
three-dimensional oriented carrier, one full-rank seed incidence induces the
common exterior closure
\[
 K_{\rm cl}=\Lambda^2J_s\oplus\Lambda^3J_s
 \quad\hbox{on}\quad
 \Lambda^2E_{\rm ax}\oplus\Lambda^3E_{\rm ax},
\]
with ranks three and one and with one Hodge metric and one action unit.  If the
seed is pointed exterior and the source-fidelity/no-bypass law is occupied,
then the induced exterior orbit is faithful and neither degree can disappear.
This closes the finite pair/top support needed by the corresponding quantum
carrier inside that enriched class.

It does not by itself derive Born probabilities, tensor factorization,
no-signalling or interacting QFT: those still require the metric--symplectic,
effect and composition assumptions proved conditionally in this paper.  Nor
does it prove that the narrow physical base--seed reduct owns the pointed
exterior grammar.  The detailed representation and occupation theorems are
owned by Technical Paper VII-B.

\subsection*{Global-law proof obligation}

The quantum packet is conditional on the globally occupied morphological
completion.  The source-fidelity/Hodge action leaves every positive-Schur
extra topology sector unoccupied because its exact reduced source satisfies
\(k_{\rm eff}=0\).  This closes occupation under that law but not Nature's
ownership of the law.  Independent component zero-jets are invisible to the
local metric--symplectic and Born packet and may reverse global selection.
Physical promotion therefore requires a deeper source derivation or a
source-frozen cross-terminal observation that distinguishes those laws.

\subsection*{Observer-relative quantum--gravity access on one complex carrier}

The common-carrier interpretation can be stated without identifying quantum
and gravity as distinct parent substances.  Let $(V_{\mathbb R},g,J)$ be the
realification of the occupied Hermitian readout carrier and let $e_Q$ and
$t_G$ be, respectively, the metric gradients of one normalized effect
probability and one smeared stress functional at the same occupied state.
Their normalized mixed response is
\[
 \sigma_{QG}=\frac{|g(e_Q,t_G)|}{\|e_Q\|_g\,\|t_G\|_g}.
\]
It lies strictly between zero and one exactly when the two gradients are
neither orthogonal nor collinear.  A stable coarse graining contracts the
effect span, so the component of a fixed stress direction visible through
quantum effects cannot increase.  The same parent direction may therefore be
predominantly quantum-visible for one observer and predominantly
gravity-visible for a coarser observer without conversion between terminal
ontologies or creation of new information.

The complex structure supplies equal-metric quadratures but does not select a
numerical overlap: $t_\theta=\cos\theta\,e_Q+\sin\theta\,Je_Q$ preserves
$(g,J)$ and realizes every value $|\cos\theta|$.  Hence neither
$1/\sqrt2$ nor any previously derived dimensionless stiffness ratio is a
quantum--gravity prediction.  Physical promotion requires source-owned
effect and stress incidences on one occupied carrier.  Standard QFT can also
realize this tangent geometry; the result is a conditional information
architecture, not a Tau-specific signal.

\section{Mixed-Hessian versus joint quantum--gravity excess}

\subsection{Inherited common-source transport and action scale}

On a faithful occupied source quotient, let $W_Q$ and $W_G$ realize the same
positive source generator $K_*$ in the quantum and gravity images.  Then
\begin{equation}
 T_{G\leftarrow Q}=W_GW_Q^{-1},\qquad
 T_{G\leftarrow Q}K_Q=K_GT_{G\leftarrow Q}
 \label{eq:papervi-qg-natural-transport}
\end{equation}
is the unique transfer compatible with both source realizations.  If
$W_Q^\dagger h_QW_Q=W_G^\dagger h_GW_G=K_*$, it is also an isometry:
$T_{G\leftarrow Q}^\dagger h_GT_{G\leftarrow Q}=h_Q$.

The quantum phase normalization in the declared common-action completion
fixes that shared unit as
\begin{equation}
 \mathcal A_*=\frac{S_0}{2\pi}=U_A=\hbar.
 \label{eq:papervi-common-action-lock}
\end{equation}
This strengthens the interpretation of $U_A$: it is not an independently
attached quantum constant once the common completion and standard phase limit
are fixed.  It remains a conditional calibration.  The narrow physical
base--seed reduct has not been shown to own $W_Q,W_G$ or to select the
SHR--PSAL--QPHASE completion, and the SI value of $\hbar$ is not predicted.

Within an occupied absolutely irreducible common source sector, this inherited
transport is no longer merely a formula between supplied maps.  Multiplicity-one
source equivariance makes each nonzero quantum or gravity map scalar-unique;
cyclic no-bypass occupation excludes the zero map, the common pullback metric
fixes the magnitude, and a source-owned oriented anchor fixes the remaining
sign.  The resulting transfer is therefore the unique source-natural isometry
inside that enriched packet.  Zero occupation, repeated equivalent copies and
an unfixed anchor are explicit controls.  Physical base--seed occupation of
the packet remains open.

The common post-body jet itself may be represented by one endpoint-blind
Schur-neutral relation-defect square without changing the stabilized
seed--body minimum.  This removes a further scalar-action ansatz freedom, but
does not derive the quantum label: rotated terminal projectors preserve the
same scalar action and change the named terminal responses.  A complete
graded source characteristic, including its upstream role representation, is
therefore the minimal sufficient object in the declared class.

The pointed exterior/CAR source compresses this condition further.  Its empty
root is cyclic on the onto no-bypass orbit, its full matrix representation is
absolutely irreducible, and no spectator copy remains.  A source-owned
oriented two-role plane would therefore supply one quantum and one gravity
copy, after which the common product metric closes the normalized Schur
fanout.  Without that typed plane, $U(2)$ role rotations preserve the untyped
source action and metric while changing the named terminal projectors; zero
legs and repeated copies remain allowed.  The remaining upstream question is
thus ownership and nonzero occupation of one oriented Q/G role incidence,
not a new quantum gain or action constant.  In the declared enriched P3 plus
Tau-local composition class, the ordered nonzero cotangent ports already
supply that oriented role plane and local tensor factorization supplies one
source copy per role.  The hidden-diagonal control shows that full Q/G
marginals without this factorization still describe only one constrained
copy.  Hence the unrestricted blocker is joint physical realization of the
known P3 port packet and locality law, not an additional quantum connector.
The pre-existing once-counted component action
$\widehat\Gamma_{\rm CLP3}=\sum_C\widehat\Gamma_C$ realizes those two
premises together at every attainable global zero. Hence the pointed-CAR,
port-factorization and Schur results close the conditional Q/G transport, and
the faithful phase branch fixes $\mathcal A_*=\hbar$, without an independent
quantum gain. Equal-reduct hidden-global and missing-port controls prevent
promoting this completion theorem to physical parent selection.
Nor can the physical question select one preferred off-shell representative:
$\Gamma_{\rm CLP3}+\lambda\Gamma_{\rm CLP3}^2$ preserves the occupied minimum,
physical Hessian and typed fanout packet.  The identifiable target is the
resulting Q/G completion class.  Hidden-global and missing-port controls alter
its defining data and therefore remain genuine class falsifiers.

The full-4D scoring compiler adds a guardrail to the common-carrier programme.
A nonzero parent mixed Hessian, including a mixed effect--stress response, is
not by itself a departure from standard quantum theory or standard gravity.
For the observer-metric branch the source-frozen comparison is
\[
 E_K=(K_{HH}-K_{\rm std})-CK_{VV}^{-1}C^\dagger,
\]
and a graph-normal auxiliary completion realizes \(C\neq0\) with \(E_K=0\).
Any future quantum--gravity discriminator must therefore push the complete
nonzero excess through independently fixed quantum and gravity terminal maps,
retain their full cross-covariance, and use a held-out fixed-hypothesis score.
The present quantum packet, QOR construction and public-data audits do not
supply such a physically occupied full-4D packet and are not rescored here.

\section{Product-free P3 falsification boundary}

For two independently reconstructed occupied P3 roles, the inherited
source-whitened law gives
\[
 C_a^2=C_b^2=I,\qquad \{C_a,C_b\}=0.
\]
Hence every real unit vector $n=(n_a,n_b)$ obeys
$(n_aC_a+n_bC_b)^2=I$. Positivity of a normalized state then yields
$|n_a\langle C_a\rangle+n_b\langle C_b\rangle|\leq1$; maximizing over $n$
proves the product-free necessary condition
\[
 \boxed{\langle C_a\rangle^2+\langle C_b\rangle^2\leq1.}
\]
A resolved violation on source-frozen independently typed roles rejects the
minimal P3 completion. Satisfaction does not confirm Tau, because an
experimenter-chosen Pauli pair satisfies the same identity in standard
quantum mechanics. The role maps must therefore be reconstructed before and
independently of the target relation. The updated Technical Paper VII-B
audit finds zero eligible packets among 31 public candidates; this is an
identifiability/acquisition result, not a quantum anomaly or a Tau pass.

\subsection*{Actional-closure handoff to the quantum terminal}

For a physically owned ordered Parent-action packet, ACT-PCLOSE1 supplies the
common regular response $D_SZ_*=-H_Z^{-1}B_{ZS}$ and hence the quantum output
as the fixed descendant
\[
 O_Q=Q_{OS,Q}\circ T_Q\circ P_Q\circ\Xi(Z_*).
\]
This removes an independent quantum response fit, but it does not derive the
metric--symplectic lock, effects, Born normalization, tensor composition or
no-signalling assumptions of the quantum packet.  Kernel completeness of the
full terminal stack is only a local reconstructibility statement; by itself
it neither produces quantum coherence nor proves Nature occupation.

\section{Conditional GNS/Gram quantum representation}

Once the observer--source descent has independently supplied a star algebra
$\mathcal A_{OS}$, a positive state $\omega_{OS}$ and a probe family $x_a$,
the GNS construction gives normalized rays and the correlation matrix
\begin{equation}
 G^{OS}_{ab}=
 \frac{\omega_{OS}(x_a^*x_b)}
 {\sqrt{\omega_{OS}(x_a^*x_a)\omega_{OS}(x_b^*x_b)}}.
 \label{eq:paperVI-gns-gram}
\end{equation}
Hermiticity, positive semidefiniteness, local ray-phase freedom and Bargmann
loop phases then follow exactly.  This is a clean representation theorem and
is compatible with the standard quantum leaf already contained in Tau Core.
It is not, by itself, a Tau-specific departure from quantum mechanics.

The representation must not be confused with the operational quantum
derivation.  A noninjective or finite-resolution readout can place distinct
Parent configurations in one ordinary outcome class, but this alone does not
give coherent superposition, a complex linear effect algebra, Born
probabilities, tensor composition or no-signalling.  Those require the
source-owned metric--symplectic/effect/tensor packet and stable quantizer
premises developed in this paper.  Likewise a Bargmann phase is a projective
holonomy only on a supported loop; it is not automatically an anyon braid,
a gravitational phase or a new observable.

The precise implication is therefore
\begin{equation}
 \begin{gathered}
 (\mathcal A_{OS},\omega_{OS},\{x_a\})
 \Longrightarrow
 (G^{OS},\text{projective phases}),\\
 (G^{OS},\text{finite loss})
 \not\Longrightarrow
 (\text{Born rule},\text{tensor locality},\text{no-signalling}).
 \end{gathered}
 \label{eq:paperVI-gram-boundary}
\end{equation}
Physical promotion still requires source ownership, the quantum terminal
map and Nature occupation; endpoint agreement cannot supply them backward.
...
The local interface is now sharper.  A differentiable oriented precursor can
survive geometrically while remaining unresolved inside the observer's stable
zero cell.  Complete record nonannihilation and the source-owned cell together
fix the first sufficient exit scale.  A nonzero outward access-projector jet
cannot be hidden from an exhaustive complete Choi instrument, although its
instantaneous probabilities can remain unchanged.  Separately, later records
intersect rather than overwrite observer-compatible Parent fibres; common
preparation and normalized future-local instruments preserve the earlier
marginal.  These results close consistency interfaces, not physical packet
occupation or a Tau-specific delayed-choice signal.
