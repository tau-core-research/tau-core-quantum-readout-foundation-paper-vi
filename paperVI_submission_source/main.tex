...
\large A finite conditional foundation for the Tau Core quantum terminal}
...
\begin{abstract}
We ask when an observer-relative, information-losing descent from an
atemporal morphological body becomes a genuine quantum terminal rather than
ordinary classical coarse graining.  The first result is a no-go statement:
finite resolution, non-injectivity and record quantization alone do not select
complex amplitudes, interference, the Born rule or no-signalling.  We then
state one finite frozen source packet sufficient for a quantum completion.  A
positive body Hessian and a compatible nondegenerate phase curvature define a
metric--symplectic lock; when its normalized endomorphism squares to minus the
identity it constructs a complex Hilbert carrier.  A commuting self-adjoint
generator gives unitary terminal transport.  Positive trace effects and
coherent composition give Born probabilities and the interference cross term.
Using the previously constructed enriched MVP/P3 handoff, we further show
that the metric, phase form, complex structure, generator, effects and common
action unit descend from one normalized complex matrix block rather than from
independent terminal assumptions.
The physical MVP closes the continuous tensor selector by requiring the
source-frozen occurrence holonomy to act irreducibly on the five-dimensional
anisotropic carrier; the nonzero seed orbit then spans the full carrier.
With the resulting tensor composition, local completely positive trace-preserving
operations preserve the remote marginal, while selective outcomes may update
conditional states without enabling signalling.  We separate record
quantization, decoherence, measurement, observer-loss entropy and the classical
limit. Exact logical recovery through a minimal qubit terminal forces
factor-pure access; a regular observer-event incidence designates the factor,
while approximate recovery bounds off-factor mixing. The central odd
Clifford sign is invisible to the resulting even $M_8$ quantum terminal. We
also import explicitly delimited standard constructions for free algebraic QFT
and perturbative causal QFT on a fixed body-derived background.  The proposed
Tau-specific discriminator states that an independently reconstructed
body-action separation must equal the Uhlmann chord of the same occupied
record pair.  No audited public carrier currently makes both sides
independently observable, so this is a conditional discriminator rather than
an executed prediction.  The quantum speed limit itself remains standard and
is used only as a downstream consistency test.
We do not prove that Nature selects the joint packet, derive the SI value of
the Planck constant, solve the outcome problem, obtain a nonperturbative QFT,
or reproduce the Standard Model.
\end{abstract}
...
It neither re-proves nor silently strengthens its inputs.
\end{minipage}}
\end{center}
...
into the squared-Hodge carrier & Exact current-source no-entailment; enriched
Nature-level occupation open \\
\bottomrule
\end{tabularx}
...
theorems and keeps Eq.~\eqref{eq:p3incidence} open at Nature level.
...
\begin{proof}
Take two stable classes $\{0,1\}$.  A classical completion is the probability
simplex $\Delta_2$ with stochastic maps.  A quantum completion is the projective
qubit carrier $\mathbb{CP}^1$ with a measurement that reports the same two
classes.  Both realize the identical ordinary quotient and the same finite
capacity.  The quantum completion additionally has a relative phase and
interference; the classical completion does not.  Hence the common quotient
data cannot select the quantum structure.  The same counterexample survives
the addition of an order on the two reported classes.  Therefore none of the
listed quantum properties follows from information loss alone.
\end{proof}
...
Eq.~\eqref{eq:matrixpacket} constructs (Q1)--(Q6) on each occupied simple
block in one source action unit.  No independent metric, phase, generator or
Born gain is required.
\end{theorem}

\begin{proof}
Faithfulness of $\tau_n$ makes $g$ positive.  Scalar multiplication gives
$J^2=-I$ and preserves $g$.  Antisymmetry of $\omega$ follows from Hermitian
symmetry, and nondegeneracy follows by choosing $Y=JX$:
$\omega(X,JX)=g(X,X)$.  For $H=H^\dagger$, left multiplication
$K_H(X)=HX$ is self-adjoint for $h$ and commutes with $J$, so
$\exp(tJK_H/\mathcal A_*)$ is unitary.  Positive matrices $\rho,E$ with
$\tau_n(\rho)=1$ and $0\le E\le I$ give
$p(E|\rho)=\tau_n(\rho E)$; positivity, normalization and coherent
interference follow from the same Hermitian pairing.  All structures are
restrictions of the trace-normalized handoff.
\end{proof}
...
This reduces the Nature-level handoff question to a finite scalar source law,
...
from the unchanged narrow parent: Nature-level selection of P3HAND-D1 and the
complete S4/S5 packet remains open.
...
as a derivation of (Q6): the physical selection of noncontextual positive
...
Nature-level closure therefore requires one remaining common-occupation
...
has the same pure blocks and zero mixed Hessian, so physical selection remains
...
Nature-level question is now whether observations support the OHC-P1 working
...
observables commute.  A positive
static occupied operator selects a compatible complex structure and pure
quasifree ground covariance; under the standard regularity hypotheses this is
a Hadamard state \citep{Radzikowski1996}.  The existing graph-normal
...
\section{Parent-selection theorem and exact boundary}
...
Nature-level input is physical base--seed occupation of one
...
preserved. Physical selection of $R_+$, its rank advantage and the common
...
metric-visible. The Nature-level statement therefore remains conditional,
...
from the explicitly frozen packet, not established as a law selected by
Nature.}
\label{tab:ledger}\\
\toprule
...
non-null metric-visible future modes; Nature-level completion selection remains
open & Conditional physical-completion theorem \\
Single-system common source & Derived inside the enriched MVP plus adopted
P3 handoff; unchanged-parent selection remains open & Conditional source
theorem \\
...
Physical selection of the completion & Zero/positive $[22]$ stiffness obeys
current source data; Nature-level six-edge occupation is not entailed & Exact
...
the full rank-five STF carrier; Nature-level OHC-P1 occupation remains open &
...
existence of a matching carrier is proved but its physical selection remains
...
falsification target, not a completed quantum theory of Nature.  Nature-level
occupation, carrier identification, effect size, nonperturbative QFT, metric
...
local Nature-level lift remains unproved rather than silently assumed.
